...
import { ArrowLeft, Save, Loader2, Trash2, MapPin, Palette, Map, LayoutGrid } from 'lucide-react';
...
export default function EditKawasanPage() {
  ...
  if (loading) {
    return (
      <>
        <div className="lg:hidden min-h-screen bg-slate-100 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            <div className="w-8 h-8 border-2 border-teal-500 border-t-transparent rounded-full animate-spin" />
            <p className="text-slate-400 text-xs">Memuatkan data kawasan...</p>
          </div>
        </div>
        <div className="hidden lg:flex min-h-screen bg-gray-50 items-center justify-center p-6">
      ...
      <div className="lg:hidden flex flex-col bg-slate-100 min-h-screen">
        ...
        <div className="bg-gradient-to-br from-[#0d7a6b] to-[#0a9e87] px-4 pt-4 pb-5 flex items-end justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-white/60 text-[10px] mt-1">{kawasan?.name || 'Kemaskini maklumat kawasan'}</p>
          </div>
          <div className="bg-white/12 border border-white/20 rounded-2xl px-3 py-3 text-center flex-shrink-0 ml-3">
            <div
              className="w-9 h-9 rounded-xl flex items-center justify-center mx-auto mb-1 border-2 border-white/30"
              style={{ backgroundColor: formData.color }}
            />
            <p className="text-white/55 text-[9px]">Warna</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 pb-28 space-y-3">
          ...
          <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
              ...
              <div className="flex-1">
                ...
                <p className="text-[9px] text-slate-400 mt-0.5">Lukis semula atau kemas kini sempadan (pilihan)</p>
              </div>
              {boundaries && (
          ...
          <div className="bg-white rounded-2xl px-4 py-4 shadow-sm">
            <p className="text-[10px] font-bold text-red-400 uppercase tracking-wide border-b border-slate-100 pb-3 mb-3">Zon Bahaya</p>
            <button
              type="button"
              onClick={handleDelete}
              disabled={deleting}
              className="w-full h-11 rounded-2xl border border-red-200 text-red-500 text-xs font-semibold flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {deleting ? (
              ...
              {deleting ? 'Memadam...' : 'Padam Kawasan Ini'}
            </button>
            <p className="text-[9px] text-slate-400 text-center mt-2">Tindakan ini tidak boleh dibatalkan</p>
          </div>
